...
const getGeminiClient = () => {
  ...
  if (!apiKey) {
    throw new Error("GEMINI_API_KEY is missing. Please set VITE_GEMINI_API_KEY in your environment/Vercel settings.");
  }
...
export default function App() {
  ...
  const handleGenerateScript = async () => {
    ...
    try {
      ...
      const parsedScript: any = await response.json();
      setScript(parsedScript);
      setActivePreviewSceneIdx(0);

      if (parsedScript._simulation) {
        setErrorToast('Fallback: Generating in simulation mode since GEMINI_API_KEY is missing.');
      }

    } catch (err: any) {
      console.error(err);
      setErrorToast(err.message || 'Script compilation failed. Check your API key or console.');
    } finally {
